...
* A **per-device realtime** helper for devices such as EV chargers (`Plants.async_get_device_realtime`), which can optionally **fall back to a `UserAuth` session** (pass `user_auth=`) when the OpenAPI per-device endpoint is unavailable for the account/region — returning the same `{uuid: {code: {...}}}` shape so callers need no changes. When the endpoint is unavailable and no fallback session is supplied it raises `DeviceEndpointUnavailable`; an empty dict means "available, but this device type has no points". `Plants.async_device_realtime_supported(...)` probes the capability once per session.
