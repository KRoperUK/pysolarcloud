"""Live probe: can a user-account session reach EMS control endpoints? (#271)

Read-only by default. Optional single idempotent write when SUNGROW_USER_WRITE_OK=1.

Run with:  pytest -m live tests/test_user_control_probe_live.py -v
...
async def test_user_control_endpoint_probe(live_user_credentials):
    """Login → device → probe control candidates; print a go/no-go summary."""
    async with UserAuth(
        ...
        report = "\n".join(lines)
        # Visible in pytest -s / CI logs without secrets.
        ...
        if probe.write_ok_enabled():
            ...
            if picked is None:
                ...
                write_result = await probe.probe_idempotent_write(
                    auth, device_uuid, path=path, label=label, set_value="0"
                )
                print(
        ...
        # Soft assertions: the spike must complete; outcome is reported for humans.
        assert results, "probe produced no results"
        assert classification in {"supported_read", "partial", "unsupported", "inconclusive"}
